        ...
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                ...
                if IsTarget:
                    ...
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, UsedList)
        Score -= 1
        if Targets[0] != -1:
            GameOver = True
        else:
...
def FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, UsedList):
    if TrainingGame:
        return [2, 3, 2, 8, 512]
    else:
        while len(NumbersAllowed) < 5:
            #here
            num = GetNumber(MaxNumber)
            print(UsedList, NumbersAllowed)
            while num in UsedList or num in NumbersAllowed:
                num = GetNumber(MaxNumber)
            NumbersAllowed.append(num)
            # here
        return NumbersAllowed
