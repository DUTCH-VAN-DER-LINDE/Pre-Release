...
class UndoState:
    def __init__(self, NumbersAllowed, Targets, Score):
        self.__NumbersAllowed = NumbersAllowed.copy()
        self.__Targets = Targets.copy()
        self.__Score = Score
    
    def GetNumbersAllowed(self):
        return self.__NumbersAllowed.copy()
...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    Score = 0
    GameOver = False
    PreviousMoveSucess = False
    state = None
    while not GameOver:
        DisplayState(Targets, NumbersAllowed, Score)
        # if the previous move was successful, allow the user to undo it
        if PreviousMoveSucess:
            print("Type undo to undo the last move.")
        UserInput = input("Enter an expression: ")
        if UserInput == "undo" and PreviousMoveSucess:
            NumbersAllowed = state.GetNumbersAllowed().copy()
            Targets = state.GetTargets().copy()
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            state = UndoState(NumbersAllowed, Targets, Score)
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                ...
                if IsTarget:
                    ...
                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber)
                    PreviousMoveSucess = True
                else:
                    PreviousMoveSuccess = False
        Score -= 1
        if Targets[0] != -1:
            GameOver = True
        else:
            Targets = UpdateTargets(Targets, TrainingGame, MaxTarget)        
        print(state.GetNumbersAllowed())
        print(state.GetTargets())
